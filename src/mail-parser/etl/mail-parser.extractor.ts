...
import { IExtractor } from '../interfaces/by-url-extractor.interface';

@Injectable()
export class MailParserExtractor {
  constructor(private readonly _fetch: FetchGet) {}

  async byUrl(url: string): Promise<IExtractor> {
  ...
  async byFilePath(filepath: string): Promise<IExtractor> {
